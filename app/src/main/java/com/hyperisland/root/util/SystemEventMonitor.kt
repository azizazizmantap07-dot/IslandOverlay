...
class SystemEventMonitor(private val context: Context) {
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            ...
            when (intent.action) {
                ...
                "android.net.wifi.WIFI_AP_STATE_CHANGED" -> {
                    ...
                    if (apOn) {
                        IslandBus.show(IslandContent.Hotspot(enabled = true), expand = false)
                    } else if (state == 11) {
                ...
                WifiManager.NETWORK_STATE_CHANGED_ACTION -> {
                    ...
                    if (lastWifi == true) {
                        IslandBus.show(buildWifiContent(enabled = true), expand = false)
                    }
                }
            }
        }
        ...
        try {
            mobileDataObserver = object : ContentObserver(mainHandler) {
                override fun onChange(selfChange: Boolean) {
                    ...
                    if (on != lastCellular) {
                        lastCellular = on
                        IslandBus.show(buildCellularContent(on), expand = false)
                    }
                }
            }
            ...
            locationObserver = object : ContentObserver(mainHandler) {
                override fun onChange(selfChange: Boolean) {
                    ...
                    if (on != lastLocation) {
                        ...
                        IslandBus.show(IslandContent.Location(enabled = on), expand = false)
                    }
                }
            }
    ...
    private fun onRingerChanged(mode: Int) {
        ...
        IslandBus.show(IslandContent.Ringer(ringer), expand = false)
    }
    ...
    private fun onDndChanged() {
        try {
            ...
            if (filter != android.app.NotificationManager.INTERRUPTION_FILTER_ALL &&
                ...
                IslandBus.show(IslandContent.Ringer(RingerMode.DND), expand = false)
            }
        } catch (t: Throwable) {
    ...
    private fun onBluetoothChanged(enabled: Boolean) {
        ...
        IslandBus.show(IslandContent.Bluetooth(enabled = enabled), expand = false)
    }
    ...
    private fun onWifiChanged(enabled: Boolean) {
        ...
        IslandBus.show(buildWifiContent(enabled), expand = false)
    }
    ...
    private fun onCellularChanged(enabled: Boolean) {
        ...
        IslandBus.show(buildCellularContent(dataOn), expand = false)
    }
    ...
    fun notifyCellularToggled(enabled: Boolean) {
        ...
        IslandBus.show(buildCellularContent(enabled), expand = true)
    }
