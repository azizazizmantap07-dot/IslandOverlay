...
class MediaSessionTracker(private val context: Context) {
    ...
    /** Guards against double start() (service restart / concurrent callers). */
    ...
    private val controllerCallback = object : MediaController.Callback() {
        ...
        override fun onSessionDestroyed() {
            ...
            lyricsKey = null
            lyricsJob?.cancel()
            // Playing media has expiresAt=0; without an explicit clear it
            // would remain on the island forever.
            IslandBus.clearMedia()
        }
    }
    ...
    private fun bindBestController(controllers: List<MediaController>?) {
        ...
        if (best != null) {
            ...
            // No active session left — stop 1Hz ticks and clear any stuck
            // Media slot (playing media never expires on its own).
            stopPositionUpdates()
            IslandBus.clearMedia()
            CrashLogger.i("No active media session — cleared island media")
        }
    }
    ...
    /** Current media card for MultiExpanded hub, or null if no active session. */
    fun currentMediaContent(): IslandContent.Media? {
        val ctrl = activeController ?: return null
        ...
        return IslandContent.Media(
            title = title,
            artist = artist ?: "",
            albumArtUri = resolveAlbumArtUri(meta, title, artist ?: ""),
            isPlaying = isPlaying,
            positionMs = position,
            durationMs = duration,
    ...
    /**
     * Resolve album art for the island backdrop:
     * 1) MediaMetadata bitmaps → [AlbumArtCache]
     * 2) MediaStyle notification largeIcon via NLS
     * 3) Metadata art URI (best-effort; may lack grant)
     */
    private fun resolveAlbumArtUri(meta: MediaMetadata?, title: String, artist: String): String? {
        if (meta == null) return null
        val cacheKey = AlbumArtCache.key(title, artist)

        // 1) Embedded bitmaps — most reliable for Spotify / local players
        val bmp: Bitmap? = try {
        ...
        // 2) MediaStyle notification largeIcon via NLS (HyperOS media card path)
        val pkg = activeController?.packageName
        val fromNotif = IslandNotificationListener.cacheArtForSession(pkg, title, artist)
        if (fromNotif || AlbumArtCache.get(title, artist) != null || AlbumArtCache.get(title, "") != null) {
            CrashLogger.d("Album art from notification for $title")
            return "cache:$cacheKey"
        }

        // 3) URI strings (YouTube Music etc.) — may fail without grant; still try
        val uri = meta.getString(MediaMetadata.METADATA_KEY_ALBUM_ART_URI)
            ...
            ?: meta.description?.iconUri?.toString()
    ...
    private fun pushToIsland() {
        val content = currentMediaContent() ?: return
        IslandBus.show(content)
        CrashLogger.d("Media push: ${content.title} – ${content.artist} playing=${content.isPlaying} lyrics=${content.lyricsLine?.take(40)}")
    }
