...
class MediaSessionTracker(private val context: Context) {
    ...
    // ---- Controls ----
    // Play/pause and skip-next/skip-previous controls were removed: the live
    // Island media card is now title + artist only, with no interactive
    // controls, so there's no caller left for transport actions here.
