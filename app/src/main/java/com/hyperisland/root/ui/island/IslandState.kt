...
sealed class IslandState {
    ...
    /**
     * Multi-card expand triggered from Minimal (or as a control hub).
     * [items] is ordered front-to-back (index 0 = frontmost card).
     * Always excludes Notification and Charging.
     */
    data class MultiExpanded(
        val items: List<IslandContent>
    ) : IslandState()
}
...
sealed class IslandContent {
    ...
    data class Ringer(
        val mode: com.hyperisland.root.system.RingerMode
    ) : IslandContent()
