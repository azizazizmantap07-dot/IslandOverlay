...
import com.hyperisland.root.xposed.FullscreenBridge
...
class IslandOverlayService : Service() {
    ...
    private val fullscreenReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action != FullscreenBridge.ACTION_FULLSCREEN_CHANGED) return
            val fs = intent.getBooleanExtra(FullscreenBridge.EXTRA_FULLSCREEN, false)
    ...
    override fun onCreate() {
        ...
        // Fullscreen / immersive detection from SystemUI LSPosed hook.
        // Must be EXPORTED so the SystemUI process broadcast can reach us.
        val fsFilter = IntentFilter(FullscreenBridge.ACTION_FULLSCREEN_CHANGED)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ContextCompat.registerReceiver(
        ...
        try { FullscreenBridge.requestState(this) } catch (_: Exception) {}
    }
    ...
    private fun handleAction(action: String) {
        ...
        when {
            ...
            action.startsWith("ringer:") -> {
                ...
                IslandBus.show(IslandContent.Ringer(mode = mode), expand = true)
                serviceScope.launch {
                    RootManager.setRingerMode(mode)
                }
            }
            action.startsWith("flashlight:") -> {
                ...
                IslandBus.show(IslandContent.Flashlight(enabled = on), expand = false)
                serviceScope.launch {
                    RootManager.setFlashlight(on)
                }
            }
            action.startsWith("bluetooth:") -> {
                val on = action.removePrefix("bluetooth:") == "on"
                serviceScope.launch {
                    RootManager.setBluetooth(on)
                }
            }
            action.startsWith("airplane:") -> {
                val on = action.removePrefix("airplane:") == "on"
                serviceScope.launch {
                    RootManager.setAirplaneMode(on)
                }
            }
            action.startsWith("wifi:") -> {
                val on = action.removePrefix("wifi:") == "on"
                IslandBus.show(IslandContent.Wifi(enabled = on), expand = true)
                serviceScope.launch {
                    RootManager.setWifi(on)
                }
            }
            action.startsWith("cellular:") -> {
                val on = action.removePrefix("cellular:") == "on"
                // Optimistic UI
                IslandBus.show(IslandContent.CellularData(enabled = on), expand = true)
                serviceScope.launch {
                    val ok = RootManager.setCellularData(on)
            ...
            action.startsWith("hotspot:") -> {
                ...
                IslandBus.show(IslandContent.Hotspot(enabled = on), expand = true)
                serviceScope.launch {
                    val ok = RootManager.setHotspot(on)
            ...
            action.startsWith("location:") -> {
                ...
                // Optimistic UI update so Expanded chips reflect immediately
                IslandBus.show(IslandContent.Location(enabled = on), expand = true)
                serviceScope.launch {
                    val ok = RootManager.setLocation(on)
                    CrashLogger.i("location toggle requested=$on success=$ok")
                    if (!ok) {
                        // Snap back if toggle failed
    ...
    override fun onDestroy() {
        ...
        try { StatusBarIconBridge.sendShowAll(this) } catch (_: Exception) {}
        serviceScope.cancel()
        super.onDestroy()
    }
