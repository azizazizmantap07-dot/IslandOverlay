...
class IslandOverlayService : Service() {
            ...
            setContent {
                ...
                androidx.compose.runtime.key(layout) {
                    OrientationAwareIsland(
                        ...
                        onExpand = {
                            when (IslandBus.state.value) {
                                is com.hyperisland.root.ui.island.IslandState.Minimal -> {
                                    val snap = com.hyperisland.root.util.SystemStateSnapshot.build(
                                        this@IslandOverlayService, mediaTracker
                                    )
                                    IslandBus.expandFromMinimal(snap)
                                }
                                else -> IslandBus.expand()
                            }
                        },
    ...
    private fun isAnyMediaPlaying(state: IslandState): Boolean {
        ...
        return when (state) {
            ...
            is IslandState.MultiExpanded -> state.items.any { playing(it) }
            is IslandState.Minimal -> false
        }
    }

    private fun refreshMultiSnapshot() {
        val snap = com.hyperisland.root.util.SystemStateSnapshot.build(this, mediaTracker)
        IslandBus.updateMultiExpanded(snap)
    }
    ...
    private fun handleAction(action: String) {
        ...
        when {
            action == "media_play_pause" -> {
                val st = IslandBus.state.value
                if (st is IslandState.MultiExpanded) {
                    val media = st.items.filterIsInstance<IslandContent.Media>().firstOrNull()
                    if (media != null) {
                        IslandBus.holdMultiOptimistic(media.copy(isPlaying = !media.isPlaying), holdMs = 800L)
                    }
                }
                mediaTracker?.togglePlayPause()
            }
            ...
            action.startsWith("ringer:") -> {
                ...
                if (IslandBus.state.value is IslandState.MultiExpanded) {
                    IslandBus.holdMultiOptimistic(IslandContent.Ringer(mode = mode))
                } else {
            ...
            action.startsWith("flashlight:") -> {
                ...
                if (IslandBus.state.value is IslandState.MultiExpanded) {
                    IslandBus.holdMultiOptimistic(IslandContent.Flashlight(enabled = on), holdMs = 2_500L)
                    com.hyperisland.root.util.SystemEventMonitor.lastKnownTorchEnabled = on
                } else {
            ...
            action.startsWith("bluetooth:") -> {
                ...
                if (IslandBus.state.value is IslandState.MultiExpanded) {
                    IslandBus.holdMultiOptimistic(IslandContent.Bluetooth(enabled = on))
                } else {
                    IslandBus.show(IslandContent.Bluetooth(enabled = on))
                }
                serviceScope.launch {
            ...
            action.startsWith("airplane:") -> {
                ...
                val airplane = IslandContent.Custom(
                    title = if (on) "Airplane mode On" else "Airplane mode Off",
                    subtitle = null
                )
                if (IslandBus.state.value is IslandState.MultiExpanded) {
                    IslandBus.holdMultiOptimistic(airplane)
                } else {
                    IslandBus.show(airplane)
                }
                serviceScope.launch {
            ...
            action.startsWith("wifi:") -> {
                ...
                if (IslandBus.state.value is IslandState.MultiExpanded) {
                    IslandBus.holdMultiOptimistic(IslandContent.Wifi(enabled = on))
                } else {
            ...
            action.startsWith("cellular:") -> {
                ...
                val inMulti = IslandBus.state.value is IslandState.MultiExpanded
                if (inMulti) {
                    IslandBus.holdMultiOptimistic(IslandContent.CellularData(enabled = on))
                } else {
                ...
                serviceScope.launch {
                    ...
                    if (inMulti || IslandBus.state.value is IslandState.MultiExpanded) {
                        refreshMultiSnapshot()
                    } else {
                        eventMonitor?.notifyCellularToggled(on)
                        if (!ok) eventMonitor?.notifyCellularToggled(!on)
                    }
                }
            }
            action.startsWith("hotspot:") -> {
                ...
                val inMulti = IslandBus.state.value is IslandState.MultiExpanded
                if (inMulti) {
                    IslandBus.holdMultiOptimistic(IslandContent.Hotspot(enabled = on))
                } else {
            ...
            action.startsWith("location:") -> {
                ...
                val inMulti = IslandBus.state.value is IslandState.MultiExpanded
                if (inMulti) {
                    IslandBus.holdMultiOptimistic(IslandContent.Location(enabled = on))
                } else {
