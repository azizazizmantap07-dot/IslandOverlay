...
- **Animasi per konten**:
  ...
  - *Location*: radar pulse + pin icon saat lokasi aktif
  - *Minimal*: **2 komet RGB berekor** yang saling mengejar searah jarum jam di tepi pill (jarak 180° tetap, ukuran head/stroke sama dengan progress ring musik, ekor ~18% perimeter, hue shift kontinu). Titik tengah breathing tetap ada di dalam. Animasi komet **selalu** aktif di state Minimal.
  - *Compact / Expanded non-musik* (notif, charging, ringer, flashlight, BT, hotspot, location, custom, atau musik yang sedang di-interrupt event lain): **full RGB ring** mengisi seluruh perimeter (mirip progress musik saat lagu habis), hue shift kontinu.
  - *Musik murni* (primary Media, tanpa secondary): progress ring RGB mengikuti posisi lagu seperti biasa.
  - Transisi: musik aktif → event live masuk → ring ganti ke full RGB; event selesai → kembali ke progress musik; semua idle → Minimal → komet mengejar.
...
| Location on/off | `SystemEventMonitor` | Compact status + On/Off di Expanded |
...
| Event | Sumber | Catatan |
|-------|--------|---------|
| Timeout auto-collapse | Compact non-media: 4s; media paused: 5s; Expanded notif: 4s | HyperOS default ~5s untuk expanded |
| Tidak ada aktivitas | Idle | Permanent island (HyperBridge) |
| User collapse | `onCollapse` / Stop Island | |

## Alur yang dipakai di IslandBus

1. `show(content)` → selalu **Compact** (kecuali notifikasi + toggle expand ON → auto Expanded)
2. Media playing: slot tidak expire. Media paused: grace 5s lalu hilang.
3. Non-media (notif, charging, ringer, …): lifetime 4s per slot; dual-slot (primary + secondary circle).
4. Tap Compact (jika toggle expand ON) → **Expanded**.
5. Expanded notifikasi: auto-dismiss 4s (ditahan saat reply field terbuka). Expanded event lain: tetap sampai swipe/aksi.
6. Timeout / collapse → **Minimal**.
